...
public class EventManager {
    ...
    private static void register(final Object o, final Class<? extends Event> clazz) {
        for (final Method method : o.getClass().getDeclaredMethods()){
    ...
    public static void unregister(final Object o, final Class<? extends Event> clazz){
        ...
        if (eventData != null && eventData.removeIf(methodData -> methodData.source() == o))
            cleanMap(true);
    }

    private static void cleanMap(boolean b) {
        if (!b) REGISTRY_MAP.clear();
        else REGISTRY_MAP.values().removeIf(List::isEmpty);
    }

    private static void removeEntry(final Class<? extends Event> clazz){
